...
# PATH LOCATIONS
# ======================================================
BASE = r"D:\Downloads\Manually_Generated_Questions_Bank\Runtime_Question_Files\L1"
EASY_FILE   = os.path.join(BASE, "Easy",   "build_l1_easy.json")
MEDIUM_FILE = os.path.join(BASE, "Medium", "build_l1_medium.json")
HARD_FILE   = os.path.join(BASE, "Hard",   "build_l1_hard.json")
...
TOPICS = {
    "easy": [
        "Percentages (basic)", "Averages (simple)", "Ratios (simple)", "Profit/Loss (basic)",
        "Time/Work (basic)", "Series (simple)", "Directions (basic)", "Blood Relations (basic)",
        "Synonyms/Antonyms (basic)", "Grammar (basic)", "Fill in the blanks"
    ],
    "medium": [
        "Percentages (growth)", "Averages (mixed groups)", "Ratios/Proportions", "Profit/Loss (cost)",
        "Time/Distance", "Series (moderate)", "Directions (multi turn)", "Blood Relations (extended)",
        "Letter Coding", "Synonyms/Antonyms (moderate)", "Grammar fixes", "Para Jumbles", "Cloze test"
    ],
    "hard": [
        "Percentages (compound)", "Averages (weighted)", "Ratios complex", "Profit/Loss (multi transaction)",
        "Time/Work/Distance", "Series (tricky)", "Directions (map logic)", "Blood Relations (puzzle)",
        "Seating Arrangement", "Mixed Coding", "Synonyms/Antonyms (nuanced)", "Grammar (complex)",
...
# ======================================================
PROMPT = """
You are generating high-quality, strictly unique Aptitude MCQs for a hiring assessment.

Your task is to generate EXACTLY {easy} EASY questions, {medium} MEDIUM questions, and {hard} HARD questions.

STRICT RULES:
- MCQ format required
- Fully unique questions
- Must match difficulty and topic
- No duplicates
- No explanations
- JSON ONLY
